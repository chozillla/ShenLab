function sim_res_err = simulation_active(par,G0,Sigma0)
...
% Sigma0(1,6) = 0.50*sigma0;
% Sigma0(6,1) = 0.50*sigma0;
...
for itrial = 1:par.Ntrials
      n = itrial; 
      % divide the procedure into the Initiation part and 
      % the Active-Learning part
      
      %Random 
      
      if itrial <= par.n_init_trials           %Turn this into a parameter like a switch 
        a = randn(6,1);
      %Active Learning
      else
        % estimate a gaussian distribution from the previously collected data points
        a_candidate = randn(6,par.n_candidates);
        Mu = mean(D((par.n_burn_in + 1):itrial-1,:))';            %mean (6 by 1)
        Sigma = cov(D((par.n_burn_in + 1):itrial-1,:));           %covariance matrix (6 by 6)
        Ent = zeros(1,par.n_candidates);            %expected entropy
 
         % draw 100 candidates a and then select one from the candidates

         for icandidate = 1:par.n_candidates     % for each a_candidate
                ...
                Ent_tmp = zeros(1,par.n_samps);
                sim_res = simulate_responseYS(a_candidate(:,icandidate), Glast, Mu, Sigma,par.n_samps);  
                
                for ires = 1:par.n_samps
                         
                         % estimate a gaussian distribution from the previously collected data points
                         G_tmp = Glast + sim_res(ires,1)+ a_candidate(:,icandidate)*sim_res(ires,2);
                         Sigma_tmp = cov([D((par.n_burn_in + 1):itrial-1,:); G_tmp']);        % re-estimate the covariance matrix

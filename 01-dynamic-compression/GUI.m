...
handles.hslider3 = uicontrol('style','slider', ...
    ...
    'Value', 0,'min', -20,'max', 20,'SliderStep',[0.025 0.025]); 
label3 = uicontrol('Parent',s,'Style','text',...
...
handles.hslider4 = uicontrol('style','slider', ...
    ...
    'Value', 0,'min', -20,'max', 20,'SliderStep',[0.025 0.025]); 
label4 = uicontrol('Parent',s,'Style','text',...
...
handles.hslider5 = uicontrol('style','slider', ...
    ...
    'Value', 0,'min', -20,'max', 20,'SliderStep',[0.025 0.025]); 
label5 = uicontrol('Parent',s,'Style','text',...
...
handles.hslider6 = uicontrol('style','slider', ...
    ...
    'Value', 0,'min', -20,'max', 20,'SliderStep',[0.025 0.025]); 
label6 = uicontrol('Parent',s,'Style','text',...
...
if stateI == 1                          %Updates the gain table in openMHA based on state of the tag
    ...
    A = ch1_ticker;
    currentChange = sprintf('change%d',A);
    t_change = now;    %Time when the slider was changed                                
    ch1_time = datetime(t_change,'ConvertFrom','datenum');%Converts it 
    ch1_change_time = ch1_time;%Records the time for the change and stores it in a matrix
    save(sprintf('ch1Value%d',A));
    A = ch1_ticker + 1; 
end
...
if stateI == 2                         %Updates the gain table in openMHA based on state of the tag
    mha_set(openmha,'mha.overlapadd.mhachain.dc.gtdata',gaintable_og);
    t_change = now;    %Time when the slider was changed   
    ch2_time = datetime(t_change,'ConvertFrom','datenum');%Converts it 
    ch2_change_time = [ch2_change_time;ch2_time]; %Records the time for the change and stores it in a matrix
end

if stateI == 3                          %Updates the gain table in openMHA based on state of the tag
    mha_set(openmha,'mha.overlapadd.mhachain.dc.gtdata',gaintable_og);
    t_change = now;    %Time when the slider was changed   
    ch3_time = datetime(t_change,'ConvertFrom','datenum');%Converts it 
    ch3_change_time = [ch3_change_time;ch3_time]; %Records the time for the change and stores it in a matrix
end

if stateI == 4                          %Updates the gain table in openMHA based on state of the tag
    mha_set(openmha,'mha.overlapadd.mhachain.dc.gtdata',gaintable_og);
    t_change = now;    %Time when the slider was changed   
    ch4_time = datetime(t_change,'ConvertFrom','datenum');%Converts it 
    ch4_change_time = [ch4_change_time;ch4_time]; %Records the time for the change and stores it in a matrix
end

if stateI == 5                          %Updates the gain table in openMHA based on state of the tag
    mha_set(openmha,'mha.overlapadd.mhachain.dc.gtdata',gaintable_og);
    t_change = now;    %Time when the slider was changed   
    ch5_time = datetime(t_change,'ConvertFrom','datenum');%Converts it 
    ch5_change_time = [ch5_change_time;ch5_time]; %Records the time for the change and stores it in a matrix
end

if stateI == 6                          %Updates the gain table in openMHA based on state of the tag
    mha_set(openmha,'mha.overlapadd.mhachain.dc.gtdata',gaintable_og);
    t_change = now;    %Time when the slider was changed   
    ch6_time = datetime(t_change,'ConvertFrom','datenum');%Converts it 
    ch6_change_time = [ch6_change_time;ch6_time]; %Records the time for the change and stores it in a matrix
end
...
elseif stateII == 2
   ...
   writematrix(ch1_change_time,'DataLog.xlsx','Range','C2');
   close all;

...
if stateI == 1                          %Updates the gain table in openMHA based on state of the tag
    gaintable_new = repmat(ch1,1,3);
    gaintable_og(1,:) = gaintable_new;
    gaintable_og(7,:) = gaintable_new;
    mha_set(openmha,'mha.overlapadd.mhachain.dc.gtdata',gaintable_og);
    t_change = now;    %Time when the slider was changed                                
    ch1time= datetime(t_change,'ConvertFrom','datenum');%Converts it 
    writematrix('250 Hz','DataLog.xlsx','Range','C1'); %Creates the label
    writematrix(ch1time,'DataLog.xlsx','WriteMode','append','Range','C2'); %Inputs the data
end

if stateI == 2                         %Updates the gain table in openMHA based on state of the tag
    gaintable_new = repmat(ch2,1,3);
    gaintable_og(2,:) = gaintable_new;
    gaintable_og(8,:) = gaintable_new;
    mha_set(openmha,'mha.overlapadd.mhachain.dc.gtdata',gaintable_og);
%     writematrix(ch2,
end

if stateI == 3                          %Updates the gain table in openMHA based on state of the tag
    gaintable_new = repmat(ch3,1,3);
    gaintable_og(3,:) = gaintable_new;
    gaintable_og(9,:) = gaintable_new;
    mha_set(openmha,'mha.overlapadd.mhachain.dc.gtdata',gaintable_og);
end

if stateI == 4                          %Updates the gain table in openMHA based on state of the tag
    gaintable_new = repmat(ch4,1,3);
    gaintable_og(4,:) = gaintable_new;
    gaintable_og(10,:) = gaintable_new;
    mha_set(openmha,'mha.overlapadd.mhachain.dc.gtdata',gaintable_og);
end

if stateI == 5                          %Updates the gain table in openMHA based on state of the tag
    gaintable_new = repmat(ch5,1,3);
    gaintable_og(5,:) = gaintable_new;
    gaintable_og(11,:) = gaintable_new;
    mha_set(openmha,'mha.overlapadd.mhachain.dc.gtdata',gaintable_og);
end

if stateI == 6                          %Updates the gain table in openMHA based on state of the tag
    gaintable_new = repmat(ch6,1,3);
    gaintable_og(6,:) = gaintable_new;
    gaintable_og(12,:) = gaintable_new;
    mha_set(openmha,'mha.overlapadd.mhachain.dc.gtdata',gaintable_og);
end

...
%Updates the gain table in openMHA based on state of the tag
    ...
    set(handles.hsliderVolume,'UserData',G);
    Data = {handles.hsliderVolume.UserData,timestr};
    mha_set(openmha,'mha.overlapadd.mhachain.dc.gtdata',gaintable_og);
...
if  stateII == 1
    ...
    writematrix('Gain','DataLog.xlsx','Range','A3');
...
elseif stateII == 2
   ...
   Gain = Data(:,1);
   Time = Data(:,2);
   writecell(Gain,'DataLog.xlsx','Range','A4:A100');
   writecell(Time,'DataLog.xlsx','Range','B4:B100');

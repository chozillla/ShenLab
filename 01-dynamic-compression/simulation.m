function sim_res_err = simulation(n_init_trials)
G0 = randn(1,6)*40-20;  %Defining Ground Truth
sigma0 = 15;
...
for itrial = 1:Ntrials
      ...
      if itrial <= n_init_trials              %Turn this into a parameter like a switch 
        ...
        Mu = mean(D(1:itrial-1,:));            %mean (1 by 6)
        ...
        Ent = zeros(1,n_candidates);                            % expected entropy
         for icandidate = 1:n_candidates  % for each a_candidate
                ...
                for ires = 1:n_samps
                         sim_res = simulate_response(a_candidate(:,icandidate)', xlim, Glast', Mu, Sigma);  
                         % estimate a gaussian distribution from the previously collected data points
                         G_tmp = Glast + sim_res(1)+ a_candidate(:,icandidate)*sim_res(2);

...
if stateI == 1                          %Updates the gain table in openMHA based on state of the tag
    ...
    handles.hslider1.UserData = [ch1 timestr];
    ...
    formatOut = 'HH:MM:SS';
    timestr = string(datestr(d,formatOut));
    handles.hslider1.UserData(end+1,1) = ch1;
    handles.hslider1.UserData(end,2) = timestr;
    data_og = handles.hslider1.UserData;
    mha_set(openmha,'mha.overlapadd.mhachain.dc.gtdata',gaintable_og);
    end
    pause(0.001);
end
...
elseif stateII == 2
   ...
   Gain1 = handles.hslider1.UserData(:,1);
   Time1 = handles.hslider1.UserData(:,2);
   writematrix(Gain1,'DataLogV2.xlsx','Range','A4:A100');
   writematrix(Time1,'DataLogV2.xlsx','Range','B4:B100');

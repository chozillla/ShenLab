...
%Updates the gain table in openMHA based on state of the tag gain
    ...
    UserData(end,3) = x;
    mha_set(openmha,'mha.overlapadd.mhachain.dc.gtdata',gaintable_og);
    save('Gain_2D.mat','UserData','-append');
    end
end
end
...
if  stateII == 1
    ...
    writematrix('Slope','DataLog_RT_2D.xlsx','Range','B3');
    writematrix('Time','DataLog_RT_2D.xlsx','Range','C3');
...
elseif stateII == 2
   ...
   Time = UserData(:,2);
   Slope = UserData(:,3);
   writematrix(Gain,'DataLog_RT_2D.xlsx','Range','A4:A1000');
   writematrix(Slope,'DataLog_RT_2D.xlsx','Range','B4:B1000');
   writematrix(Time,'DataLog_RT_2D.xlsx','Range','C4:C1000');
   close all;

function dragpoints_2_v11(fname)
global openmha
global file_cont % flag to determine that is session is a continuation
global max_pins % number of pins for first intial trial
global data
global pin_count % related to the total number of individual "trials"
                 % this is not considered a trial since the user has not 
                 % fully confirmed it 
...
if nargin == 1
  ...
  yLower = -20;
  yUpper = 20;
  load(fname,'data','pin_count','trial_num')
  file_cont = 1;
  disp("file_flagged")
end
...
handles.htxt = uicontrol('style','text', ... % this is the textbox that shows the current trial number 
    'unit', 'normalized', ...
    'position' , [.35 .93 .3 .05], ...
    'String', sprintf('Trial %d out of 25',(trial_num - 25)), ...
    'fontsize', 28);
...
%      mha_set(openmha,'cmd','stop');
    ...
    elseif stateII == 2 % Next button
        ...
        if file_cont == 1
            ...
            disp(trial_num)
            handles.htxt.String = sprintf('Trial %d out of 25',(trial_num - 25));
        else
            flag = cell2mat(data(5,:));
            index = find(flag == 2);
            trial_num = length(index);
